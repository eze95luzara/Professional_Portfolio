...
export default function Tabla(props){
    const onClickEliminar = async(id) => {
        if (window.confirm("Desea eliminar el entrenador?"))
            props.onEliminar(id)
    }
    ...
    const tdata = props.rows.map((e) => {
        return (
            <tr key={e.entrenador_id}>
                ...
                <td>
                    <button className="btn btn-danger" disabled={e.eliminado === true} onClick={() => { onClickEliminar(e.entrenador_id) }}>Eliminar</button>
                </td>
                <td>
        ...
        <>
            <div className="card">
                <div className="card-header d-flex justify-content-between align-items-center">
                    <h6>Entrenadores</h6>
                    <div>
                        <button className="btn btn-success me-2" onClick={props.onRegistro}>Nuevo Entrenador</button>
