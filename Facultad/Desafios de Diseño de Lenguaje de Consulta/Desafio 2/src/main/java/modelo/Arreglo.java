...
public class Arreglo
{
    ...
    private class MergeSort implements Ordenable
    {
        ...
        public void ordenar()
        {
            int n = v.length;
            temp = new int[n];
            int inver = sort(0, n - 1, 0);
            System.out.println(inver);
        }
        
        private int sort(int izq, int der, int inver)
        {
           int inverIzq;
           int inverDer;
           int inverMerge;
            ...
            if(izq < der) 
            {
                ...
                inverIzq = sort(izq, centro, inver);
                inverDer = sort(centro + 1, der, inver);
                inverMerge = merge(izq, centro, der, inver);
                
                inver = inverIzq + inverDer + inverMerge;
            }
            
            return inver;
        }       
        
        private int merge(int izq, int centro, int der, int inver) throws ArrayIndexOutOfBoundsException
        {
    ...
    /**
     *  Genera un arreglo con valores obtenidos de un archivo. 
     */
    public void generar (String nomArch) {
        int i = 0;   
