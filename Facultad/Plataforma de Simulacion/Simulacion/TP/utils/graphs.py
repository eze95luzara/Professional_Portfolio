...
def histogram(numbers, intervals):
    histogram = create_histogram(numbers, intervals)
    flike = io.BytesIO()
    histogram.savefig(flike)
    b64 = base64.b64encode(flike.getvalue()).decode()
    return b64
    
def create_histogram(numbers, intervals):
    plt.close() #En caso de nueva consulta, se borra el plt anterior
    ax = plt.subplot(111)
    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    esperados = frecuencias_esperadas(len(numbers), intervals)
    combinado = [numbers, esperados]
    n, bins, patches = plt.hist(combinado, bins=intervals, alpha=.5, range=(0,1), label=['F Obs','F Esp'])
