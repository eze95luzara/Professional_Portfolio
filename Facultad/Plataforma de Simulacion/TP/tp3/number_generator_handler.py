...
class TP3NumberGeneratorHandler(NumberGeneratorHandler):
    ...
    def build_observed_frequencies(self):
        ...
        if self.intervals is None:
            ...
            self.intervals = self.generate_intervals(self.number_generator, self.intervals_amount, last_item['minimum'], last_item['maximum'])
    ...
    @staticmethod
    def generate_intervals(number_generator, intervals_amount, lower_limit, upper_limit):
    ...
    @staticmethod
    def count_observed_frequency(generated_number, observed_frequencies):
        for interval, observed_frequency in observed_frequencies.items():
            lower_limit, upper_limit = interval
            if lower_limit <= generated_number and generated_number < upper_limit:
    ...
    def run(self):
        ...
        for item in self.history:
            self.frequencies = self.count_observed_frequency(item['generated_number'], self.frequencies)
