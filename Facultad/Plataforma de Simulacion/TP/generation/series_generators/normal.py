...
class NormalNumberGenerator(BaseSeriesGenerator):
    def __init__(self,
        ...
        if variance is not None:
            ...
            self.standard_deviation = float(variance) 
        else:
    ...
    def generate_number(self):
        ...
        for i in range(12):
            ...
            randoms_list.append(self.truncate(random_number, self.decimal_places))
            sum_random_numbers += random_number
            combined_random = sum_random_numbers - 6 
        #Metodo convolucion con k=12
        # [sumatoria(RND) -6]*desviacion estandar + media
        #self.random_number = self.truncate(sum_random_numbers, self.decimal_places)
        self.random_number = combined_random
        return self.truncate(combined_random * self.standard_deviation + self.mean, self.decimal_places)
